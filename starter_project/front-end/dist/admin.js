...
$(document).ready(function () {
	...
	__webpack_require__(/*! ../less/admin.less */ "./src/less/admin.less");
	__webpack_require__(/*! ./resources.js */ "./src/js/resources.js");

	var token = localStorage.getItem("token");
	var navLinkButton = ".mdl-navigation__link";
	var pageContent = ".mdl-layout__content .page-content";
	var addButton = ".mdl-button.add-newUser";
	var removeButton = ".mdl-button.mdl-js-button.mdl-button--raised.mdl-button--colored.remove-button";
	var sendEditedButton = ".save-changes-button";
	var resetButton = ".mdl-button.mdl-js-button.mdl-button--raised.mdl-button--colored.reset-button";
	var searchInput = ".users-management .mdl-textfield__input";
	var subjectsInput = ".add-user .mdl-textfield__input#subjects";
	var errorMsg = "Auch!!! Ceva nu a mers bine!";
	var notification = document.querySelector("#notification");
	var removeUserId = void 0;
	var resetUserId = void 0;
	var usersData = void 0;
	var subjectsData = void 0;
	var subjectsList = [];
	var subjectsListUser = [];
	var subjectsListIds = [];
	var subjectsListUserIds = [];

	// const hostName = '0.0.0.0:2222';
	var hostName = '89.34.92.135:2222';
	var urlAddUser = 'http://' + hostName + '/api/admin/users?token=' + token;
	var urlGetUsers = 'http://' + hostName + '/api/admin/users?token=' + token;

	var urlGetSubjects = 'http://' + hostName + '/api/all_subjects?token=' + token;
	...
	function apiAllSubjectsGet(callback) {
		...
		$.get(urlGetSubjects).done(function (data) {
	...
	function apiSendRemoveUserPost(data, callback) {
		console.log(data);
		var urlPostRemoveUser = 'http://' + hostName + '/api/admin/users/' + data.id + '/delete?token=' + token;
		$(".loader-bck").show();

		$.post(urlPostRemoveUser).done(function (data) {
	...
	function apiSendEditedPost(data, callback) {
		// console.log(data);
		var urlPostEditUser = 'http://' + hostName + '/api/admin/users/' + data.id + '/update?token=' + token;
		$(".loader-bck").show();
	...
	function apiResetUserPasswordPost(data, callback) {
		...
		var urlPostResetPassword = 'http://' + hostName + '/api/admin/users/' + data.id + '/reset?token=' + token;
		// console.log(data);
		$(".loader-bck").show();
		$.post(urlPostResetPassword, data).done(function (data) {
	...
	function apiAllUsersGet(callback) {
		...
		$.get(urlGetUsers).done(function (data) {
	...
	function getSuggestions(value) {
		...
		return inputLength <= 1 ? [] : usersData.filter(function (user) {
	...
	function renderUsers(array) {
		...
		array.map(function (user, index) {
			...
			for (var i in user.subject_ids) {
				for (var j in subjectsData) {
		...
		array.map(function (user, index) {
			...
			$('#edit-subjects-' + user.id).on("focus click input", function () {
				...
				var searchText = $('#edit-subjects-' + user.id).val();

				if (searchText.length !== 0) {
					var _array = getSuggestions_subjects(searchText);

					if (_array.length !== 0) {
						renderSubjectsUser(_array, index);
					} else {
						$('#content-dropdown-' + index + ' .demo-list-control.mdl-list.dropdown').hide();
					}
				} else {
		...
		$(sendEditedButton).on('click', function () {
			...
			console.log(id);
			var user = $(this).parent().parent().attr("userIndex");

			var fullName = $('.mdl-textfield__input#edit-fullName-' + id);
			// const userName = $(".mdl-textfield__input#edit-userName").val();
			var mail = $('.mdl-textfield__input#edit-email-' + id);
			var subjects = $('.mdl-textfield__input#edit-subjects-' + id);

			subjectsListUserIds = [];
			// for (let i in subjectsListUser[user]){
			// 	subjectsListUserIds.push(subjectsListUser[user][i].id_subject);
			// }
			subjectsListUserIds = subjectsListUser[user].map(function (s) {
				return s.id_subject;
			});
			console.log(subjectsListUserIds);

			var cond3 = verifyInput(subjects, subjectsListUserIds.length !== 0 ? /[\s\S]*/ : /(?!)/, 'edit-subjects-req-' + id, "Adaugati cel putin o materie");

			var cond2 = verifyInput(mail, /^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/, 'edit-email-req-' + id, "Introduceti o adresa de email valida");
			var cond1 = verifyInput(fullName, /^[a-zA-Z\s-]+$/, 'edit-fullName-req-' + id, "Folositi doar litere separate prin spatiu sau '-'");

			if (cond1 && cond2 && cond3) {

				var obj = { fullName: $(fullName).val(), mail: $(mail).val(), id_subjects: subjectsListUserIds, id: id };
				console.log(obj);
		...
		function renderSubjectsUser(array, user) {
			...
			console.log(array);
			array.map(function (subj, index) {

				$('#content-dropdown-' + user + ' .demo-list-control.mdl-list.dropdown').append('<li class="mdl-list__item" key=' + subj.id + '>\n\t\t\t\t\t\t<span class="mdl-list__item-primary-content">\n\t\t\t\t\t\t\t' + subj.name + '\n\t\t\t\t\t\t</span>\n\t\t\t\t\t</li>');
			...
			$('#content-dropdown-' + user + ' .dropdown .mdl-list__item').on("click", function () {
				...
				var id_subject = $(this).attr("key") * 1;
				val = val.trim();

				var isMultiple = false;
				for (var i in subjectsListUser[user]) {
				...
				if (isMultiple === false) {
					subjectsListUser[user].push({ val: val, id_subject: id_subject });
	...
	$(searchInput).on('input', function () {
		...
		if (searchText.length !== 0) {
			var array = getSuggestions(searchText);
	...
	$(navLinkButton).on('click', function () {
		...
		if (linkId === "data-export") {
			...
			var urlExport = 'http://' + hostName + '/api/export/?token=' + token;
	...
	$(addButton).on('click', function () {
		...
		var subjects = $(".content-input #subjects");
		...
		var cond3 = verifyInput(subjects, subjectsListIds.length !== 0 ? /[\s\S]*/ : /(?!)/, "subjects-req", "Adaugati cel putin o materie");

		var cond2 = verifyInput(email, /^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/, "email-req", "Introduceti o adresa de email valida");

		var cond1 = verifyInput(fullName, /^[a-zA-Z\s-]+$/, "fullname-req", "Folositi doar litere separate prin spatiu sau '-'");

		// const cond2 = subjectsListIds.length !== 0 ? true : false;

		if (cond1 && cond2 && cond3) {

			var obj = { fullName: $(fullName).val(), mail: $(email).val(), id_subjects: subjectsListIds };
	...
	function getSuggestions_subjects(value) {
		...
		// console.log(subjectsData);
		return inputLength <= 1 ? [] : subjectsData.filter(function (subj) {
			return subj.name.toLowerCase().slice(0, inputLength) === inputValue;
		});
	}
	...
	function renderSubjects(array) {
		...
		array.map(function (subj, index) {
			$(".content-dropdown .demo-list-control.mdl-list.dropdown").append('<li class="mdl-list__item" key=' + subj.id + '>\n\t\t\t\t\t<span class="mdl-list__item-primary-content">\n\t\t\t\t\t' + subj.name + '\n\t\t\t\t\t</span>\n\t\t\t\t</li>');
		});
		...
		$(".content-dropdown .dropdown .mdl-list__item").on("click", function () {
			...
			if (isMultiple === false) {
				subjectsList.push({ val: val, id_subject: id_subject });
	...
	function renderSubjectsList(list) {
		...
		list.map(function (subj, index) {

			$(".page-content .subjects-list .list").append('<div class="mdl-chip mdl-chip--deletable">\n          \t\t\t<div class="mdl-chip__text">' + subj.val + '</div>\n          \t\t\t<button type="button" class="mdl-chip__action" id=' + subj.id_subject + '>\n          \t\t\t\t<i class="material-icons">cancel</i>\n          \t\t\t</button>\n        \t\t</div>');
		});
	...
	$(".add-user .container").on("click", function (event) {
		if (!event.target.matches(".content-dropdown .demo-list-control.mdl-list.dropdown") && $(".content-dropdown .demo-list-control.mdl-list.dropdown").has(event.target).length === 0 && !event.target.matches("#subjects")) {
			$(".content-dropdown .demo-list-control.mdl-list.dropdown").hide();
		}
	});

	$(subjectsInput).on("focus click input", function () {
		var searchText = $(subjectsInput).val();

		if (searchText.length !== 0) {
			var array = getSuggestions_subjects(searchText);
...
$(document).ready(function () {
    __webpack_require__(/*! ../less/resources.less */ "./src/less/resources.less");

    var url = "http://89.34.92.135:2222";
    // const url = "http://0.0.0.0:2222";
    var token = localStorage.getItem("token");

    var resources = [];
    var moreMenusList = [];
    var moreMenusListRemoverEnabled = false;

    var createCard = function createCard(json) {
        var body = document.createElement("div");
        var moreMenuContainer = document.createElement("div");
        var moreMenu = document.createElement("i");
        var classImageContainer = document.createElement("div");
        var classImage = document.createElement("i");
        var titleContainer = document.createElement("div");
        var titleImg = document.createElement("i");
        var title = document.createElement("span");
        var typeContainer = document.createElement("div");
        var typeImg = document.createElement("i");
        var type = document.createElement("span");
        var capacityContainer = document.createElement("div");
        var capacityImg = document.createElement("i");
    ...
    var createAddCard = function createAddCard() {
    ...
    var createMoreMenuCard = function createMoreMenuCard(parent, resId) {
        var body = document.createElement("div");
    ...
    var createUpdateCard = function createUpdateCard(resId) {
        var resource = resources.find(function (item) {
            return item["resource"]["id"] === resId;
        });
        var bodyOverlay = document.createElement("div");
        var body = document.createElement("div");
        var title = document.createElement("h1");
    ...
    var createYesNoCard = function createYesNoCard(resId) {
        var bodyOverlay = document.createElement("div");
        var body = document.createElement("div");
        var title = document.createElement("h1");
        var form = document.createElement("form");
    ...
    var updateResource = function updateResource(_ref2) {
        var id = _ref2.id,
            type = _ref2.type,
            name = _ref2.name,
    ...
    var deleteResource = function deleteResource(_ref3) {
        var id = _ref3.id;
    ...
    var updateAllResources = function updateAllResources() {
    ...
    var closeMoreMenus = function closeMoreMenus() {
        moreMenusList.forEach(function (element) {
            element.parentNode.removeChild(element);
        });
        moreMenusList = [];
    };

    var closeFormWindowHandler = function closeFormWindowHandler(event) {
        if (event.which === 27) {
    ...
    var closeMenusHandler = function closeMenusHandler(event) {
        if (moreMenusList !== []) {
